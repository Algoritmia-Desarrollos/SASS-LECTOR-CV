...
function crearFila(postulacion) {
    ...
    row.innerHTML = `
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-sm font-semibold text-gray-900">${candidato?.nombre_candidato || 'N/A'}</div>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
            <button data-action="ver-notas" title="Notas" class="text-gray-500 hover:text-gray-800"><i class="fa-solid fa-note-sticky"></i></button>
            <button data-action="ver-cv" title="Descargar CV" class="text-gray-500 hover:text-gray-800"><i class="fa-solid fa-download"></i></button>
        </td>
    `;
...
async function abrirModalNotas(candidatoId) {
    ...
    if (notas.length === 0) {
        notesHistoryContainer.innerHTML = '<p class="text-gray-500">No hay notas para este candidato.</p>';
    } else {
        notesHistoryContainer.innerHTML = notas.map(n => `
            <div class="bg-gray-50 p-3 rounded-md">
                ...
                <p class="text-xs text-gray-500 mt-2">
                    ...
                    ${n.postulacion ? `(en la búsqueda: <strong>${n.postulacion.aviso.titulo}</strong>)` : ''}
                </p>
            </div>
        `).join('');
    }
